...
def calcular(df: pd.DataFrame, filtro: dict) -> dict:
    ...
    if tipo == "ultimas_vendas" and filtro.get("cliente"):
        ...
        if 'NUM_DOCTO' in dff.columns:
            ...
            resumo_notas = []
            # LOG: verificar valores de NUM_DOCTO
            sample = grp['NUM_DOCTO'].head(5).tolist()
            logging.warning(f"[RESUMO_NOTAS] NUM_DOCTO sample: {sample} | busca_produto={filtro.get('busca_produto')}")
